...
class CustomTrainer(train.BaseTrainer):
    ...
    def __init__(self, args):
        ...
        # other_tools.load_checkpoints(self.model, self.args.data_path_1 +  "pretrained_vq/rvq_hands_500.bin", args.e_name)
        ...
        for its, dict_data in enumerate(self.train_loader):
            ...
            if self.args.rec_ver_weight > 0:
                ...
                if (
                    ...
                    vertices_vel_loss = self.vel_loss(
                        vertices_rec["vertices"][:, 1:]
                        - vertices_rec["vertices"][:, :-1],
                        vertices_tar["vertices"][:, 1:]
                        - vertices_tar["vertices"][:, :-1],
                    ) * self.args.rec_weight
                    vertices_acc_loss = self.vel_loss(
