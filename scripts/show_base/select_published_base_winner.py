    ...
    *,
    ...
    for expected_epoch, raw_row in zip(
        ...
        if (
            ...
            or updates != epoch * authority.BASE_UPDATES_PER_EPOCH
        ):
            raise PublishedBaseSelectionError(
...
def build_published_base_winner_selection(
    ...
    *,
    ...
    The evidence artifact contains no selectable metric scalar.  Ranking FGD
    comes only from the separately byte-pinned primary replay receipt after
    the neutral pure verifier binds its prediction files, extractor/cache,
    metric assets, runtime and report.  The report's own scalar never ranks.
    """
    ...
    if (
        evidence["format"] != CANDIDATE_EVIDENCE_FORMAT
    ...
    result: dict[str, Any] = {
        "format": authority.WINNER_SELECTION_FORMAT,
