...
talkshow_feature_extractor=
talkshow_validation_gate=
talkshow_validation_gate_sha=
talkshow_validation_gate_payload_sha=
source_audio_root=
smplx_path=
seed=20260801
...
while (($#)); do
    case "$1" in
        ...
        --talkshow-feature-extractor)
            talkshow_feature_extractor=$2; shift 2 ;;
        --talkshow-validation-gate-json)
            talkshow_validation_gate=$2; shift 2 ;;
        --expected-talkshow-validation-gate-sha256)
            talkshow_validation_gate_sha=$2; shift 2 ;;
        --expected-talkshow-validation-gate-receipt-payload-sha256)
            talkshow_validation_gate_payload_sha=$2; shift 2 ;;
        --source-audio-root)
        ...
        --talkshow-torch-threads)
            talkshow_torch_threads=$2; shift 2 ;;
        --)
            shift
            (($# == 0)) || {
...
for value_name in \
    ...
    diffsheg_root talkshow_root talkshow_metric_root \
    talkshow_feature_extractor talkshow_validation_gate \
    talkshow_validation_gate_sha talkshow_validation_gate_payload_sha \
    source_audio_root smplx_path; do
...
for digest in \
    ...
    "$talkshow_validation_gate_sha" \
    "$talkshow_validation_gate_payload_sha"; do
    [[ $digest =~ ^[0-9a-f]{64}$ ]] || {
...
[[ $seed =~ ^[0-9]+$ && $diffsheg_batch_size =~ ^[1-9][0-9]*$ && \
   $talkshow_torch_threads =~ ^[1-9][0-9]*$ ]] || {
    printf 'seed/metric batch/thread contract mismatch\n' >&2
    exit 2
}
...
for required in \
    ...
    "$talkshow_metric_root" "$talkshow_feature_extractor" \
    "$talkshow_validation_gate" \
...
talkshow_feature_extractor=$(realpath -e -- "$talkshow_feature_extractor")
talkshow_validation_gate=$(realpath -e -- "$talkshow_validation_gate")
...
diffsheg_preflight=$log_root/combined-metrics-preflight.json
diffsheg_output=$output_root/combined-final-metrics
...
diffsheg_asset_args=(
    ...
    --talkshow-feature-extractor "$talkshow_feature_extractor"
    --talkshow-validation-gate-json "$talkshow_validation_gate"
    --expected-talkshow-validation-gate-sha256 \
        "$talkshow_validation_gate_sha"
    --expected-talkshow-validation-gate-receipt-payload-sha256 \
        "$talkshow_validation_gate_payload_sha"
    --talkshow-torch-threads "$talkshow_torch_threads"
    --batch-size "$diffsheg_batch_size"
)

# CPU-only preflight consumes no test metric allowance.  It is published once,
# externally pinned below, and replayed by the sole combined metric process.
preflight_result=$(
...
[[ $diffsheg_preflight_sha =~ ^[0-9a-f]{64}$ && \
   $diffsheg_audio_set_sha =~ ^[0-9a-f]{64}$ ]] || {
    printf 'combined metric preflight did not return exact external pins\n' >&2
    exit 1
}
[[ ! -e $diffsheg_output && ! -L $diffsheg_output ]] || {
    printf 'combined output already exists; refusing a second test event\n' >&2
    exit 1
}

# This is the sole formal test-metric process.  It performs both required
# suites over the shared final manifest under one already-consumed claim;
# there is no separately launched TalkSHOW evaluator.
pids=()
starttimes=()
argv_hashes=()
trap cleanup_children EXIT
...
if [[ -z $metric_snapshot ]]; then
    printf 'cannot attest the sole combined metric child\n' >&2
    exit 1
fi
...
if ! wait "$metric_pid"; then
    printf 'the sole combined test event failed; no retry is allowed\n' >&2
    exit 1
fi
pids=()
starttimes=()
...
terminal_validation=$log_root/combined-final-result-validation.json
validation_result=$(
...
[[ ${#validation_fields[@]} == 5 && \
   ...
   ${validation_fields[4]} =~ ^[0-9a-f]{64}$ ]] || {
    printf 'terminal combined result validator did not PASS\n' >&2
    exit 1
}
...
if value.get("format") != "semtalk_show_combined_final_result_validation_v1" or value.get("status") != "complete":
...
printf 'SemTalk SHOW combined final test complete: %s\n' \
    "$final_metrics_path"
