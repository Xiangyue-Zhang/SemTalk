...
class CustomTrainer(train.BaseTrainer):
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        mask_val[:, :self.args.pre_frames, :] = 0.0  #  需要前四帧作为seed pose

        net_out_val  = self.model(
            loaded_data['beat'], loaded_data['in_word'], mask=mask_val,
            in_id = loaded_data['tar_id'], in_motion = loaded_data['latent_all'],
            use_attentions = True,hubert=loaded_data['hubert'], is_train=True)
        
        g_loss_final = 0

        ##### rec loss #####
        loss_latent_face       = self.reclatent_loss(net_out_val["rec_face"], loaded_data["zq_face"])
        ...
        tar_index_value_upper_top = loaded_data["tar_index_value_upper_top"].reshape(-1,6)
        tar_index_value_lower_top = loaded_data["tar_index_value_lower_top"].reshape(-1,6)
        tar_index_value_hands_top = loaded_data["tar_index_value_hands_top"].reshape(-1,6)
        for i in range(6):
            rec_index_face_val = self.log_softmax(net_out_val["cls_face"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
            rec_index_upper_val = self.log_softmax(net_out_val["cls_upper"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
            rec_index_lower_val = self.log_softmax(net_out_val["cls_lower"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
            rec_index_hands_val = self.log_softmax(net_out_val["cls_hands"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
        ...
        #     # ------ masked gesture moderling------ #
            ...
            net_out_self  = self.model(
                loaded_data['beat'], loaded_data['in_word'], mask=mask,
                in_id = loaded_data['tar_id'], in_motion = loaded_data['latent_all'],
                use_attentions = True, use_word=False, hubert=loaded_data['hubert'], is_train=True)
            
            loss_latent_face_self = self.reclatent_loss(net_out_self["rec_face"], loaded_data["zq_face"])
            ...
            for j in range(6):
                ...
                rec_index_upper_self = self.log_softmax(net_out_self["cls_upper"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
                rec_index_lower_self = self.log_softmax(net_out_self["cls_lower"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
                rec_index_hands_self = self.log_softmax(net_out_self["cls_hands"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
            ...
            net_out_word  = self.model(
                loaded_data['beat'], loaded_data['in_word'], mask=mask,
                in_id = loaded_data['tar_id'], in_motion = loaded_data['latent_all'],
                use_attentions = True, use_word=True,hubert=loaded_data['hubert'], is_train=True)

            loss_latent_face_word = self.reclatent_loss(net_out_word["rec_face"], loaded_data["zq_face"])
            ...
            for i in range(6):
                ...
                rec_index_upper_word = self.log_softmax(net_out_word["cls_upper"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
                rec_index_lower_word = self.log_softmax(net_out_word["cls_lower"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
                rec_index_hands_word = self.log_softmax(net_out_word["cls_hands"][:,:,:,i]).reshape(-1, self.args.vae_codebook_size)
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(0, roundt):
            ...
            net_out_val = self.model(
                ...
                in_id = in_id_tmp,
                hubert=hubert_tmp,
                use_attentions=True, is_train=False)
            

            rec_index_upper = self.log_softmax(net_out_val["cls_upper"]).reshape(-1, self.args.vae_codebook_size,6)
            _, rec_index_upper = torch.max(rec_index_upper.reshape(-1, 16, self.args.vae_codebook_size,6), dim=2)
        

            rec_index_lower = self.log_softmax(net_out_val["cls_lower"]).reshape(-1, self.args.vae_codebook_size,6)
            _, rec_index_lower = torch.max(rec_index_lower.reshape(-1, 16, self.args.vae_codebook_size,6), dim=2)
        

            rec_index_hands = self.log_softmax(net_out_val["cls_hands"]).reshape(-1, self.args.vae_codebook_size,6)
            _, rec_index_hands = torch.max(rec_index_hands.reshape(-1, 16, self.args.vae_codebook_size,6), dim=2)
        

            rec_index_face = self.log_softmax(net_out_val["cls_face"]).reshape(-1, self.args.vae_codebook_size,6)
